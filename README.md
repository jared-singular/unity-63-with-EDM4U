...
This project is a **Unity 6.3** sample app that demonstrates how to integrate the **External Dependency Manager for Unity (EDM4U)** with an iOS project that uses **CocoaPods**, and how to add the **Singular Unity SDK** on top.
...
- Show a clean way to:
  - Install and configure **EDM4U** via Unity Package Manager.
  - Generate and manage an iOS **Podfile** and CocoaPods workspace from Unity.
  - Integrate the **Singular Unity SDK** using EDM4U and CocoaPods.
...
- macOS with:
  - **Xcode** installed.
  - **CocoaPods** installed and working from Terminal (`pod --version`).
  - A recent **Homebrew Ruby** (recommended) if you want to avoid system Ruby / `ffi` issues.
...
- EDM4U is added as a Unity package (not as a legacy `.unitypackage`).
- The iOS Resolver is enabled to:
  - Generate a `Podfile` under `Builds/`.
  - Add CocoaPods integration to the Xcode workspace.
...
The **Singular Unity SDK** is installed via Git URL in Unity Package Manager.

High‑level setup:

1. Add the Singular package to the project via **Window → Package Manager → Add package from Git URL…**.
2. Add the Singular iOS dependency to EDM4U so it appears in the generated `Podfile` (the Podfile will include `Singular-SDK`).
...
2. **Automatic CocoaPods step**
   ...
   - This:
     ...
     - Installs or updates the `Singular-SDK` pod.
     - Creates `Unity-iPhone.xcworkspace` in `Builds/`.
